<template>
	<div :class="[
		...
		{ 'bg-black flex-1': isVideo }
	]">
		<!-- 文件夹 -->
		<img v-if="file.type === 0" :src="IconFolder" alt="file preview" class="w-auto h-full object-cover p-2" />
		<template v-else>
			<!-- 图片 -->
			<img v-if="fileType === 'image'" :src="OSS_IMAGE_RESIZE + file.full_path" alt="file preview"
				class="w-full object-cover p-2" />
			<!-- audio  -->
			<img v-if="fileType === 'audio'" :src="IconAudio" alt="file preview"
				class="w-auto h-full object-cover p-2" />
			<!-- video -->
			...
			<div v-if="fileType === 'video' && file.preview" class="relative w-full h-full group">
				...
				<div
					...
					<div class="w-12 h-12  flex items-center justify-center">
						<svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-white filter drop-shadow-md"
			...
			<img v-if="fileType === 'document'" :src="IconDocument" alt="file preview"
				class="w-auto h-full object-cover p-2" />
			<!-- code -->
			<img v-if="fileType === 'code'" :src="IconCode" alt="file preview" class="w-auto h-full object-cover p-2" />
			<!-- zip -->
			<img v-if="fileType === 'zip'" :src="IconZip" alt="file preview" class="w-auto h-full object-cover p-2" />
			<!-- unknown -->
			<img v-if="fileType === 'unknown'" :src="IconUnknown" alt="file preview"
				class="w-auto h-full object-cover p-2" />
		</template>
