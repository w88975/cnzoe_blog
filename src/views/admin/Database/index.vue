				...
				<table class="min-w-full border border-gray-300">
					...
					<tbody>
						<tr v-for="(column, index) in tableStructure" :key="column._key" class="hover:bg-gray-50">
							...
							<td class="border border-gray-300 px-4 py-2">
								<input v-model.lazy="column.dflt_value"
									class="border border-gray-300 rounded-md w-full px-2 py-1"
									@change="updateColumn(column)" />
							</td>
							<td class="border border-gray-300 px-4 py-2">
								<button @click="addBefore(index)"
									class="text-blue-600 hover:text-blue-800 underline mr-2">往前新增</button>
								<button @click="addAfter(index)"
									class="text-blue-600 hover:text-blue-800 underline mr-2">往后新增</button>
								<button @click="removeColumn(index)"
									class="text-red-600 hover:text-red-800 underline">删除</button>
							</td>
						</tr>
					</tbody>
				</table>
