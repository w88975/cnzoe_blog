...
export async function uploadFileToR2(file, onProgress, dir = '') {
    return new Promise((resolve, reject) => {
        ...
        let lastLoaded = 0
        // 生成随机数
        const randomNum = `${Math.floor(Math.random() * 10000)}_${Date.now()}`
        ...
        xhr.onload = async () => {
            if (xhr.status === 200) {
                ...
                // 构造新的文件名
                const newFileName = `${file.name.split('.')[0]}_${randomNum}.${fileExtension}`
                // 构造相对 URL
                const relativeUrl = `${dir}/${newFileName}`
                resolve({
        ...
        // 构造新的文件名
        const newFileName = `${file.name.split('.')[0]}_${randomNum}.${fileExtension}`
        let key = dir ? `${dir}/${newFileName}` : newFileName
